export const SHELVES = [
  ...
  {
    id: 'stream', title: 'Stream Processing',
  ...
  {
    id: 'storage', title: 'Storage at Scale',
...
export const TECHNOLOGIES = [
  {
    ...
    emoji: '⚡',
    color: '#dc2626',
    tagline: 'In-memory key-value. Caching, pub/sub, Lua atomic ops.',
    facts: [
      'Single-threaded — one hot key saturates an entire core',
      'RESP protocol handles ~100k ops/sec per CPU core',
      'SETEX + Lua scripts for atomic check-and-decrement',
    ],
    concepts: ['cache-aside', 'thundering-herd', 'hot-key', 'token-bucket', 'sliding-window', 'fan-out-on-write'],
    launchSlug: 'cache-aside',
  ...
  },
  {
    ...
    emoji: '🌊',
    color: '#2563eb',
    tagline: 'Distributed commit log. Partitions = parallelism ceiling.',
    facts: [
      'Consumers per group ≤ partition count — extra consumers idle',
      'Consumer group rebalance triggers a lag spike every time',
      'Committing offsets after processing = at-least-once delivery',
    ],
    concepts: ['kafka-partitions', 'at-least-once-delivery', 'offset-management'],
    launchSlug: 'kafka-partitions',
    scenario: 'stream_proc',
  },
  {
    id: 'postgres',
    name: 'PostgreSQL',
    emoji: '🐘',
    color: '#0369a1',
    tagline: 'MVCC + B-tree indexes. Pool exhaustion shows up before CPU.',
    facts: [
      'max_connections is hard process limit — pgBouncer sits in front',
      'EXPLAIN ANALYZE: seq scan vs index scan shows the bottleneck',
      'VACUUM required for dead tuple reclaim after heavy deletes',
    ],
    concepts: ['connection-pool-exhaustion', 'two-phase-commit', 'saga-pattern', 'idempotency-keys'],
    launchSlug: 'connection-pool-exhaustion',
    scenario: 'url_shortener',
  },
  {
    id: 'cassandra',
    ...
    emoji: '💎',
    color: '#7c3aed',
    tagline: 'LSM trees. Write path: memtable → SSTable → compaction storm.',
    facts: [
      'Writes always land in memtable first — fast by design',
      'Compaction merges SSTables periodically — I/O spike is expected',
      'Tunable consistency: ONE / QUORUM / ALL per operation',
    ],
    concepts: ['lsm-tree', 'backpressure'],
    launchSlug: 'lsm-tree',
    scenario: 'write_scaling',
  },
  {
    id: 'elasticsearch',
    name: 'Elasticsearch',
    emoji: '🔍',
    color: '#059669',
    tagline: 'Inverted index + BM25. Refresh interval = write-to-read lag.',
    facts: [
      'Shards are immutable — plan count before indexing, you cannot split later',
      'Default refresh_interval=1s means 1s write-to-search latency',
      'BM25 scoring weights by TF-IDF across all shards',
    ],
    concepts: ['inverted-index', 'relevance-scoring', 'shard-routing'],
    launchSlug: 'inverted-index',
    scenario: 'search',
  },
  {
    id: 'flink',
    ...
    emoji: '🌀',
    color: '#b45309',
    tagline: 'Stateful stream processing. Checkpoints ≠ Kafka offsets.',
    facts: [
      'RocksDB state backend for large state, heap backend for small/fast',
      'Watermarks determine window completeness for event-time processing',
      'Exactly-once = aligned checkpoints + idempotent sinks together',
    ],
    concepts: ['at-least-once-delivery', 'offset-management', 'kafka-partitions'],
    launchSlug: 'at-least-once-delivery',
    scenario: 'stream_proc',
  },
  {
    id: 's3',
    ...
    emoji: '🪣',
    color: '#d97706',
    tagline: 'Get blobs off your app server. Presigned URLs + multipart.',
    facts: [
      'Presigned URL = time-limited auth token for direct client upload',
      'Multipart parts must be ≥5MB each, parallelizable across threads',
      'Block direct S3 access — route everything through signed URLs',
    ],
    concepts: ['presigned-urls', 'multipart-upload'],
    launchSlug: 'presigned-urls',
    scenario: 'blob_store',
  },
]

export const ALL_CONCEPTS = SHELVES.flatMap(s => s.concepts)
