...
DelaySeconds only applies when sending a _new_ message. For messages already in-flight (which is our case), use ChangeMessageVisibility to defer reprocessing. Visibility timeout can be extended up to 12 hours per message.
...
You might wonder how we can achieve thousands of pages per second when we're rate-limited to 1 request/second per domain. The key insight is that we're crawling _millions_ of different domains in parallel. Each crawler maintains thousands of concurrent connections to different sites simultaneously, so while any single domain sees at most 1 request/second, the aggregate throughput across all domains is very high.
