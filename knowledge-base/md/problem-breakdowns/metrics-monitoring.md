...
The best approach takes advantage of the fact that most queries are covering the same data. Like the sliding window problem, queries that are separated by 10s are covering _almost completely overlapping data_. The only difference is the last 10s.
