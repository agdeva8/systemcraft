...
FeatureUDPTCPConnectionConnectionlessConnection-orientedReliabilityBest-effort deliveryGuaranteed deliveryOrderingNo ordering guaranteesMaintains orderFlow ControlNoYesCongestion ControlNoYesHeader Size8 bytes20-60 bytesSpeedFasterSlower due to overheadUse CasesStreaming, gaming, VoIPEverything Else
...
Typically the application layer is processed in "User Space" whereas layers beneath it are processed in the OS kernel in "Kernel Space". This means that the application layer is more flexible and can be more easily modified than lower layers, whereas lower layers are difficult to change but can be _very_ efficient.
...
In practice, most collaborative editors _don't_ require scaling to thousands of clients. Additionally, you often need a central server anyways to store the document and coordinate between clients. That's why we're using Websockets in our Google Docs problem breakdown. But there is an alternative to use WebRTC and CRDTs (Conflict-free Replicated Data Types) to achieve a truly peer-to-peer experience.
